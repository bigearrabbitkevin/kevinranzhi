...
?>
<div class='show'><?php echo $searchForm ?></div>
		...
		<div class='side-body'>
			<div class='panel panel-sm'>
				...
				<div class='panel-body'>
					<?php echo $deptTree; ?>
					<div class='text-right'><?php commonModel::printLink('dept', 'browse', '', $lang->kevinuser->common); ?></div>
				</div>
			</div>
		</div>
	</div>
			...
			<form action='<?php echo $this->createLink('kevinuser', 'batchEdit', "deptID=$deptID") ?>' method='post' id='userListForm'>
				<table class='table table-condensed table-hover table-striped tablesorter table-fixed  table-selectable' id='userList'>
					...
					<tr class='colhead'>
						...
						<th class='w-id'><?php commonModel::printorderlink('id', $orderBy, $vars, $lang->kevinuser->idAB); ?></th>
					...
					<?php foreach ($users as $user):
						...
						<tr class='text-center'>
							...
							<td <?php echo $lockbackground; ?>><?php if (!commonModel::printLink('kevinuser', 'profile', "account=$user->account", $user->realname)) echo $user->realname; ?></td>
							<td><?php
							...
							<td class='text-left'><?php echo html::a($this->createLink('kevinuser', 'edit', "userID=$user->id&from=company"), $lang->kevinuser->edit); ?>
								...
								if ($isNotAdmin) {
									...
									if ((int)$user->locked == 0) {
										echo html::a($this->createLink('kevinuser', 'userLock', "userID=$user->account"), "<i class='icon-unlock-alt text-muted'></i>", 'hiddenwin', "class='btn-icon' title={$titlelockedNone}", false);
									} else if ($user->locked < $longLockTime) {
										echo html::a($this->createLink('kevinuser', 'unlock', "userID=$user->account"), "<i class='icon-lock'></i>", 'hiddenwin', "class='btn-icon' title={$titlelockedTemp}", false);
									} else {
